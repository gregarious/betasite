from scenable.outsourcing.models import ICalendarFeed, VEventRecord
from scenable.places.models import Location, Place
from scenable.events.models import Event, Role

from scenable.outsourcing.places import smart_text_resolve
...
logger = logging.getLogger('scenable.outsourcing')
...
class FeedImporter(object):
    ...
    def import_new(self, start_filter=None):
        ...
        def _process_time(component):
            ...
            # if couldn't find one, return the dt localized to the server's TZ with a notice
            if not tz_str:
                notices.append(EventImportReport.UnavailableTimezone())
                return localize(component.dt)

            # we have a timezone string, try localizing it. if the string is invalid, return a notice
            try:
