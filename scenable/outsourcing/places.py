...
from scenable.outsourcing.apitools.factual import FactualAPIError
from scenable.outsourcing.apitools import geocoding_client_factory, factual_client_factory
from scenable.places.models import Place, Location

import logging
resolvelog = logging.getLogger('scenable.resolve')
...
def resolve_location(partial_location, allow_numberless=True, retry=None):
    ...
    fields = [partial_location.address, partial_location.town, state]
    ...
    for field in fields:
        ...
        if m:
            ...
            try:
                ...
                if 'potential_state' in result_dict:
                    result_dict.pop('potential_state')
        m = postcode_pattern.search(field)
        if m:
            result_dict['potential_postcode'] = m.group(1)
