...
def geocoding_client_factory(retry=None):
    return google.GoogleGeocodingClient(retry=retry)


def factual_client_factory(retry=None):
    return factual.FactualClient(factual.OIP_OAUTH_KEY, factual.OIP_OAUTH_SECRET, retry=retry)
