...
from scenable.outsourcing.apitools import facebook, facebook_client
from scenable.outsourcing.apitools.facebook import FacebookAPIError
from scenable.outsourcing import fbpages
from scenable.events import categorize
from scenable.places.models import Place, Location
from scenable.events.models import Event, Role

from scenable.outsourcing.models import FacebookEventRecord, FacebookOrgRecord, ExternalPlaceSource
from scenable.outsourcing.places import resolve_place, resolve_location
from scenable.settings import TIME_ZONE
from scenable.common.utils.time import localize
...
outsourcing_log = logging.getLogger('scenable.outsourcing')
...
def store_fbevent(event_info, event_image=None,
    ...
    event.dtend = dtend_local
    # process image
    if event_image is None:
        try:
            event.image_url = facebook_client.graph_api_picture_request(fbid)
        except IOError as e:
            outsourcing_log.error('Error retreiving picture for event %s: %s' % (unicode(fbid), unicode(e)))
    else:
...
class EventImportManager(object):
    ...
    def pull_event_info_from_pages(self, page_ids, start_filter=None, use_cache=True):
        ...
        try:
            ...
            if not use_cache:
                return dict([(pid, []) for pid in page_ids])
        ...
        page_events_map = dict([(pid, [eid_fbevent_map[eid] for eid in eids])
